...
from dinov2.eval.utils import ModelWithIntermediateLayers, HFModelWithIntermediateLayers, evaluate, evaluate_chexpert_test
...
def evaluate_linear_classifiers(
    ...
    per_class_auc_labels = {k: v for k, v in zip(data_loader.dataset.dataset.pathologies, per_class_auc)}
...
def eval_linear(
    *,
    ...
    for batch in metric_logger.log_every(
        ...
        losses = {f"loss_{k}": nn.BCEWithLogitsLoss()(v, labels) for k, v in outputs.items()}
...
def run_eval_linear(
    ...
    sample_input = sample_input.pixel_values.squeeze(0) if 'pixel_values' in sample_input else sample_input
    ...
    val_data_loader = make_eval_data_loader(val_dataset_config, batch_size, num_workers, val_metric_type)
    ...
    results_dict[f"VAL_macro"] = 100.0 * val_results_dict["best_classifier"]["macro"]
...
def main(args):
    ...
    train_data_loader = make_data_loader(
        ...
        num_workers=args.args.num_workers,
    )
