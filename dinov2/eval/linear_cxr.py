...
def setup_linear_classifiers(sample_output, n_last_blocks_list, learning_rates, batch_size, num_classes=1000):
    ...
    for n in n_last_blocks_list:
        for avgpool in [True]: # Why for loop?
    ...
    logger.info("="*50)
    logger.info("Training the following classifiers: \n")
    all_clf_names = list(linear_classifiers_dict.keys())
    for i,clf_name in enumerate(all_clf_names):
        logger.info(f"#{i+1}) {clf_name} \n")
    logger.info("="*50)
...
def eval_linear(
    *,
    feature_model,
    linear_classifiers_list, # linear_classifiers,
    train_data_loader,
    val_data_loader,
    metrics_file_path,
    optimizers_list,
    ...
    checkpointers_list = []
    for i in range(len(linear_classifiers_list)):
        checkpointers_list.append(Checkpointer(linear_classifiers_list[i], output_dir, optimizer=optimizers_list[i], scheduler=schedulers_list[i]))
    # checkpointer = Checkpointer(linear_classifiers, output_dir, optimizer=optimizer, scheduler=scheduler)

    for checkpointer in checkpointers_list:
        start_iter = checkpointer.resume_or_load(classifier_fpath or "", resume=resume).get("iteration", -1) + 1
    
    validation_trackers = [ValidationTracker() for i in range(len(linear_classifiers_list))] # val tracker for each experiment
    ...
    for batch in metric_logger.log_every(
        ...
        labels = labels.squeeze(-1).long() if isinstance(criterion, nn.CrossEntropyLoss) else labels
        features = feature_model(data)

        # compute the outputs for each experiment, backprop
        for i, linear_classifiers in enumerate(linear_classifiers_list):
            ...
            schedulers_list[i].step()

        # log
        if iteration % 10 == 0:
            torch.cuda.synchronize()
            metric_logger.update(loss=loss.item()) # only log the first exp loss
            metric_logger.update(lr=optimizers_list[0].param_groups[0]["lr"]) # print lr for the first one (doesn't matter, they're all the same)
            print("lr", optimizers_list[0].param_groups[0]["lr"]) 
        ...
        if eval_period > 0 and (iteration + 1) % eval_period == 0 and iteration != max_iter - 1:
            ...
            for i, linear_classifiers in enumerate(linear_classifiers_list):
                ...
                val_results_dict_list.append(val_results_dict)

            for i, val_results_dict in enumerate(val_results_dict_list):
                val_macro_score = val_results_dict["best_classifier"]["macro"]
                best_classifier_name = val_results_dict["best_classifier"]["name"]
                if val_macro_score > validation_trackers[i].get_best_score():
                    if distributed.is_main_process():
                        torch.save(linear_classifiers_list[i].state_dict(),os.path.join(checkpointer.save_dir,f"best_model_{i}_dict.pth"))
                        torch.save(linear_classifiers_list[i].module.classifiers_dict[best_classifier_name].state_dict(), os.path.join(checkpointer.save_dir,f"best_model_{i}.pth"))
                validation_trackers[i].update(val_macro_score, iteration,val_results_dict_list[i])
    ...
    for i, val_results_dict in enumerate(val_results_dict_list):
        ...
        validation_trackers[i].update(val_macro_score, iteration,val_results_dict)
    
    for i, validation_tracker in enumerate(validation_trackers):
        logger.info(f"Best validation score head {i}: {validation_tracker.get_best_score()} from iteration {validation_tracker.get_best_iteration()}, best classifier: {validation_tracker.get_best_score_dict()['best_classifier']['name']}")
    
    best_val_results_dict_list = [validation_tracker.get_best_score_dict() for validation_tracker in validation_trackers]
    
    torch.distributed.barrier()

    linear_classifiers_list = [_load_model(model=linear_classifiers_list[i], path=os.path.join(checkpointer.save_dir,f"best_model_{i}_dict.pth")) for i in range(len(linear_classifiers_list))]
    
    return best_val_results_dict_list, feature_model, linear_classifiers_list, iteration
...
def test_on_datasets(
    ...
    for config, class_mapping, metric_type in zip(test_dataset_config, test_class_mappings, test_metric_types):
        ...
        i = 0
        while os.path.exists(metrics_file_path.replace("results_eval_linear.json", f"preds_{config.dataset_name}_{i}_rank_{rank}.npy")):
            i += 1
        np.save(metrics_file_path.replace("results_eval_linear.json", f"preds_{config.dataset_name}_{i}_rank_{rank}.npy"), preds)
        np.save(metrics_file_path.replace("results_eval_linear.json", f"gt_{config.dataset_name}_{i}_rank_{rank}.npy"), gt)
...
def run_eval_linear(
    ...
    if train_dataset.dataset_name == 'CHSC' and training_num_classes>1:
        criterion = nn.CrossEntropyLoss()
        logger.info("Using CrossEntropyLoss for Multi Class Classification on CHSC dataset")
    else:
    ...
    # TODO: @PM: Remove hardcoded values from here
    ...
    linear_classifiers_list = []
    optimizers_list = []
    schedulers_list = []

    logger.info(f"===> Preparing {num_experiments} experiments...")
    for i in range(num_experiments):
        ...
        linear_classifiers_list.append(linear_classifiers)

        ## TODO: @PM: Remove hardcoded values for weight decay, and sgd-momentum
    ...
    val_data_loader = make_eval_data_loader(val_dataset_config, batch_size, 0, val_metric_type) # number of workers=0 #@PM: Debug, why?
    ...
    # Train and Val loop
    val_results_dict_list, feature_model, linear_classifiers_list, iteration = eval_linear(
        feature_model=feature_model,
        linear_classifiers_list=linear_classifiers_list,
    ...
    results_dict_list = []
    for i,linear_classifiers in enumerate(linear_classifiers_list):
        logger.info(f"---Testing classifier {i}---")
        results_dict = test_on_datasets(
            ...
            training_num_classes,
            iteration,
            val_results_dict_list[i]["best_classifier"]["name"],
            prefixstring="",
    ...
    results_dict["Average_VAL_macro"] = np.mean([val_results_dict['best_classifier']['macro'] for val_results_dict in val_results_dict_list])
    results_dict["STD_VAL_macro"] = np.std([val_results_dict['best_classifier']['macro'] for val_results_dict in val_results_dict_list])

    results_dict["Average_Test_macro"] = np.mean([results_dict[f"TEST_{test_dataset_name}_macro"] for results_dict in results_dict_list])
    results_dict["STD_Test_macro"] = np.std([results_dict[f"TEST_{test_dataset_name}_macro"] for results_dict in results_dict_list])
    results_dict["Test_macro_per_head"] = [results_dict[f"TEST_{test_dataset_name}_macro"] for results_dict in results_dict_list]
    logger.info("Test Results Dict " + str(results_dict))
    logger.info(f"Average Test Macro over {num_experiments} experiments: {results_dict['Average_Test_macro']}")
    logger.info(f"STD Test Macro over {num_experiments} experiments: {results_dict['STD_Test_macro']}")


    if f'TEST_{test_dataset_name}_macro_5_findings' in results_dict_list[0]:
        results_dict["Average_Test_macro_5_findings"] = np.mean([results_dict[f"TEST_{test_dataset_name}_macro"] for results_dict in results_dict_list])
        results_dict["STD_Test_macro_5_findings"] = np.std([results_dict[f"TEST_{test_dataset_name}_macro_5_findings"] for results_dict in results_dict_list])
        results_dict["Test_macro_5_findings_per_head"] = [results_dict[f"TEST_{test_dataset_name}_macro_5_findings"] for results_dict in results_dict_list]
        logger.info(f"Average Test Macro 5 findings over {num_experiments} experiments: {results_dict['Average_Test_macro_5_findings']}")
        logger.info(f"STD Test Macro 5 findings over {num_experiments} experiments: {results_dict['STD_Test_macro_5_findings']}")
...
def main(args):
    ...
    print("="*50)
    print(f"Working with optimizer: {optimizer_str}")
    print("="*50)
