...
def collate_data_and_cast_cxr(samples_list, mask_ratio_tuple, mask_probability, dtype, n_tokens=None, mask_generator=None):
    ...
    # print(samples_list[]['dataset'])
    collated_ds_name_local = [s['dataset'] for _ in range(n_local_crops) for s in samples_list]
    collated_ds_name_global = [s['dataset'] for _ in range(n_global_crops) for s in samples_list]
    collated_ds_name = [s['dataset'] for s in samples_list]
    ...
    return {
        ...
        "upperbound": upperbound,
        "n_masked_patches": torch.full((1,), fill_value=mask_indices_list.shape[0], dtype=torch.long),
        "ds_local": collated_ds_name_local,
        "ds_global": collated_ds_name_global,
        "ds": collated_ds_name
    }